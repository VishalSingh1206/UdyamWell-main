...
const SingleCoursePage = () => {
    ...
    useEffect(() => {
        const handleFullScreenChange = () => {
          setIsFullScreen((document.fullscreenElement || document.webkitFullscreenElement) !== null);
        };
    ...
    return (
      ...
      <ReactPlayer
        ...
        playing={isPlaying}
        volume={volume}
        onProgress={(progress) => setPlayed((progress.played * 100).toFixed(2))}
  ...
  {playerRef.current && (
    <>
      <span>{formatTime(playerRef.current.getCurrentTime())}</span>
      /
  ...
  <div className="other-controls">
      ...
      <button onClick={handlePlayPause}>
        ...
        {isMuted ? <VolumeOffIcon sx={{ fontSize: 36 }} /> : <VolumeUpIcon sx={{ fontSize: 36 }} />}
      </button>{showVolumeSlider && (
         <div className="volume-slider">
            ...
            sx={{ fontSize: 36 }}
              ...
              max={1}
              step={0.01}
              style={{ width: '90px', transform: 'rotate(0deg)', marginLeft: '10px' }}
            />
            </div>)}</div>
        </div>
